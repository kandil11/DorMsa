...
const EditListing = () => {
    ...
    const fetchListing = async () => {
      try {
        ...
        setForm({
          ...
          period: data.price?.period || 'monthly',
          address: data.location?.address || '',
          area: data.location?.area || '',
          distanceFromCampus: data.location?.distanceFromCampus || '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payloadData = {
        ...
        removeImages: JSON.stringify(removeIndices)
      };
  ...
  if (loading) return <div className="p-10"><Skeleton className="w-full h-96 rounded-2xl" /></div>;

  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
      <h1 className="text-2xl font-extrabold text-secondary-700 mb-6">Edit Listing</h1>
      <Card hover={false} className="max-w-3xl p-6">
        <form onSubmit={handleSubmit} className="space-y-5" encType="multipart/form-data">
          ...
          <div className="space-y-1.5">
            ...
            <textarea rows={4} value={form.description} onChange={(e) => handleChange('description', e.target.value)} className="w-full px-4 py-2.5 bg-white border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary-500/30 focus:border-primary-500 resize-none" />
          </div>

          <div className="space-y-3">
            <label className="block text-sm font-medium text-secondary-700">Current Photos (Select to remove)</label>
            <div className="flex flex-wrap gap-3">
              {existingImages.map((img, idx) => (
                <div key={idx} className="relative group" onClick={() => toggleRemoveImage(idx)}>
                  <img 
                    src={img.index !== undefined ? `${API_URL}/listings/${id}/image/${img.index}` : img.url} 
                    className={`w-24 h-24 object-cover rounded-lg border-2 ${removeIndices.includes(idx) ? 'border-red-500 opacity-50' : 'border-transparent'}`} 
                    alt="Property" 
                  />
                  ...
                  <div className="absolute inset-0 bg-black/0 group-hover:bg-black/10 transition-colors cursor-pointer rounded-lg" />
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="space-y-1.5">
            <label className="block text-sm font-medium text-secondary-700">Add New Photos</label>
            ...
            {newImages.length > 0 && <p className="text-xs text-primary-600 mt-1">{newImages.length} new file(s) selected</p>}
          </div>
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            <Input label="Price (EGP) *" type="number" value={form.price} onChange={(e) => handleChange('price', e.target.value)} />
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            <Input label="Area" value={form.area} onChange={(e) => handleChange('area', e.target.value)} />
            <Input label="Distance from Campus (km)" type="number" value={form.distanceFromCampus} onChange={(e) => handleChange('distanceFromCampus', e.target.value)} />
          </div>
          <Input label="Contact Phone" type="tel" value={form.contactPhone} onChange={(e) => handleChange('contactPhone', e.target.value)} />
          
          <div className="flex gap-3 pt-2">
            <Button type="submit" loading={saving} icon={Save}>Update Listing</Button>
